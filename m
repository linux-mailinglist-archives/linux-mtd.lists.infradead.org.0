...
Received: from bombadil.infradead.org (bombadil.infradead.org [IPv6:2607:7c80:54:e::133])
	by mail.lfdr.de (Postfix) with ESMTPS id E69AF8D2DF
	for <lists+linux-mtd@lfdr.de>; Wed, 14 Aug 2019 14:19:32 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; q=dns/txt; c=relaxed/relaxed;
	...
	List-Owner; bh=ELra5K2P7FucaInLb4Yk3IOMF2wVDqcFPq6dgH9ISKg=; b=X1DGg1yl4H9dp9
	;
Received: from localhost ([127.0.0.1] helo=bombadil.infradead.org)
	by bombadil.infradead.org with esmtp (Exim 4.92 #3 (Red Hat Linux))
	id 1hxsFU-00014j-0O; Wed, 14 Aug 2019 12:19:28 +0000
Received: from metis.ext.pengutronix.de ([2001:67c:670:201:290:27ff:fe1d:cc33])
 by bombadil.infradead.org with esmtps (Exim 4.92 #3 (Red Hat Linux))
 id 1hxsEu-0000XO-Gm
 for linux-mtd@lists.infradead.org; Wed, 14 Aug 2019 12:18:56 +0000
Received: from dude.hi.pengutronix.de ([2001:67c:670:100:1d::7])
 by metis.ext.pengutronix.de with esmtps
 (TLS1.3:ECDHE_RSA_AES_256_GCM_SHA384:256) (Exim 4.92)
 (envelope-from <sha@pengutronix.de>)
 id 1hxsEt-0005kK-4Z; Wed, 14 Aug 2019 14:18:51 +0200
Received: from sha by dude.hi.pengutronix.de with local (Exim 4.92)
 (envelope-from <sha@pengutronix.de>)
 id 1hxsEm-00083M-Qp; Wed, 14 Aug 2019 14:18:44 +0200
From: Sascha Hauer <s.hauer@pengutronix.de>
To: linux-fsdevel@vger.kernel.org
Subject: [PATCH 11/11] ubifs: Add quota support
Date: Wed, 14 Aug 2019 14:18:34 +0200
Message-Id: <20190814121834.13983-12-s.hauer@pengutronix.de>
...
X-CRM114-CacheID: sfid-20190814_051853_197621_9D093FAF 
...
index e2d3dfb67ae3..b7a2c255ffcd 100644
--- a/fs/ubifs/dir.c
+++ b/fs/ubifs/dir.c
@@ -113,6 +113,11 @@ struct inode *ubifs_new_inode(struct ubifs_info *c, struct inode *dir,
 			 current_time(inode);
...
index 400970d740bb..a2800d56217f 100644
--- a/fs/ubifs/file.c
+++ b/fs/ubifs/file.c
...
+			dquot_free_space_nodirty(inode, quota_size);
 		...
 		/*
@@ -1112,6 +1130,7 @@ static int do_truncation(struct ubifs_info *c, struct inode *inode,
 	int err;
...
@@ -1191,6 +1210,10 @@ static int do_truncation(struct ubifs_info *c, struct inode *inode,
...
+		dquot_free_space_nodirty(inode, quota_size);
 ...
 out_budg:
 	if (budgeted)
@@ -1227,6 +1250,17 @@ static int do_setattr(struct ubifs_info *c, struct inode *inode,
...
@@ -1278,6 +1312,15 @@ int ubifs_setattr(struct dentry *dentry, struct iattr *attr)
 	if (err)
 		return err;
...
index 824def711381..0805665ce0b4 100644
--- a/fs/ubifs/ioctl.c
+++ b/fs/ubifs/ioctl.c
@@ -157,16 +157,48 @@ static int ubifs_ioctl_check_project(struct ubifs_inode *ui, struct fsxattr *fa)
 	return 0;
 }
...
index 000000000000..88351c227975
--- /dev/null
+++ b/fs/ubifs/quota.c
@@ -0,0 +1,590 @@
...
index dc7e235b1a4a..3f20866b73db 100644
--- a/fs/ubifs/super.c
+++ b/fs/ubifs/super.c
...
@@ -344,6 +345,7 @@ static void ubifs_evict_inode(struct inode *inode)
...
+	dquot_initialize(inode);
 	...
 	if (err)
@@ -353,7 +355,7 @@ static void ubifs_evict_inode(struct inode *inode)
 		 */
...
@@ -363,6 +365,7 @@ static void ubifs_evict_inode(struct inode *inode)
 		smp_wmb();
 	}
 done:
+	dquot_drop(inode);
...
@@ -425,6 +428,9 @@ static int ubifs_show_options(struct seq_file *s, struct dentry *root)
...
@@ -913,6 +919,13 @@ static int check_volume_empty(struct ubifs_info *c)
 	return 0;
 }
...
@@ -1090,6 +1105,15 @@ static int ubifs_parse_options(struct ubifs_info *c, char *options,
 			break;
 		case Opt_ignore:
 			break;
+#ifdef CONFIG_QUOTA
...
+#endif
 		...
 		{
 			unsigned long flag;
@@ -1786,6 +1810,7 @@ static int ubifs_remount_rw(struct ubifs_info *c)
 		 */
...
+	c->vfs_sb->s_flags &= ~SB_RDONLY;
 	...
 	return err;
@@ -1855,6 +1880,7 @@ static void ubifs_remount_ro(struct ubifs_info *c)
 	err = dbg_check_space_info(c);
 	if (err)
 		ubifs_ro_mode(c, err);
+	c->vfs_sb->s_flags |= SB_RDONLY;
...
@@ -1865,6 +1891,8 @@ static void ubifs_put_super(struct super_block *sb)
...
@@ -1962,11 +1990,17 @@ static int ubifs_remount_fs(struct super_block *sb, int *flags, char *data)
...
@@ -1997,6 +2031,9 @@ const struct super_operations ubifs_super_operations = {
...
+#endif
 ...
 /**
@@ -2157,6 +2194,8 @@ static int ubifs_fill_super(struct super_block *sb, void *data, int silent)
 #endif
...
+
 	...
 	err = mount_ubifs(c);
 	if (err) {
@@ -2171,6 +2210,10 @@ static int ubifs_fill_super(struct super_block *sb, void *data, int silent)
 		goto out_umount;
 	}
...
index fd9e52749ef7..884fce6c133e 100644
--- a/fs/ubifs/ubifs.h
+++ b/fs/ubifs/ubifs.h
@@ -27,6 +27,7 @@
...
@@ -2108,6 +2118,23 @@ static inline bool ubifs_crypt_is_encrypted(const struct inode *inode)
